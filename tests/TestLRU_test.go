...
func TestLRUFunctionality(t *testing.T) {
	...
	g1, _ := gedis.NewGedis(gedis.Config{Address: addresses[0]})
	g2, _ := gedis.NewGedis(gedis.Config{Address: addresses[1]})
	...
	g3, _ := gedis.NewGedis(gedis.Config{Address: addresses[2]})
	g3.Set("key3", "value3")
	...
	g4, _ := gedis.NewGedis(gedis.Config{Address: addresses[3]})
	g4.Set("key4", "value4")
